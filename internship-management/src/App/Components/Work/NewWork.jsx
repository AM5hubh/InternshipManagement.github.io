...
export default function NewWork() {
  ...
  const [selectedInternsListHelperText, updateSelectedInternsListHelperText] =
    useState("");
  ...
  function getData(parameter = "") {
    fetchInternsData(parameter).then((array) => {
      ...
      updateInternsList((oldData) => {
        let newData = array
          .filter((intern) => {
            // removing interns who's internship is completed
            console.log(intern);
            const endDate = new Date(intern.hireDetails.toDate).setHours(
              0,
              0,
              0,
              0
          ...
          .map((data) => {
            return {
              id: data._id,
              name: data.fullName + " - " + data.emailID,
              email: data.emailID,
              type: "intern",
            };
          });
          ...
          >
            ...
            <div>
              <Autocomplete
                ...
                renderInput={(params) => (
                  <TextField
                    {...params}
                    error={selectedInternsListHelperText || false}
                    helperText={selectedInternsListHelperText || ""}
                    label={`Members (${selectedOptions.length})`}
                    placeholder="List of interns / groups"
                  />
                )}
              />
            </div>
            ...
            <div>
              ...
              <TextField
                ...
                step={0.1}
                onChange={updateForm}
                helperText="Multiplier for early completion bonus (1.0 = normal, 2.0 = double bonus)"
                sx={{ ml: 2 }}
                InputProps={{
